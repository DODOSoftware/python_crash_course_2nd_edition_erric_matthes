# Paul 10.05.2022 - please don't mind excessive comments
# - it's just an exercise purpose :)

polish_rivers = ['wisla', 'odra', 'warta', 'bug', 'narew', 'san', 'notec',
                 'pilica', 'wieprz', 'dunajec', 'bobr', 'lyna', 'nysa luzycka',
                 'wkra', 'brda', 'prosna', 'drweca', 'wislok', 'wda', 'drawa',
                 'nysa klodzka', 'poprad', 'pasleka', 'rega', 'bzura',
                 'wisloka', 'obra', 'lega', 'biebrza', 'wierzyca', 'nida',
                 'gwda', 'czarna hancza', 'liwiec', 'orzyc', 'wieprza',
                 'barycz', 'parseta', 'slupia', 'kamienna', 'ner',
                 'mala panew', 'raba', 'omulew', 'kwisa', 'ina', 'krzna',
                 'wełna', 'radomka', 'skrwa', 'elk', 'wel', 'radunia', 'szkwa',
                 'netta', 'suprasl', 'nurzec', 'olawa']
...
# Paul 10.05.2022 - New list for adding, removing and modifying.
print('\nNEW LIST\n')

print('Home Gym equipment list - print raw:')

# Paul 10.05.2022 - created emtpy list and adding by .append()
...
# Paul 10.05.2022 - modifying items
...
home_gym_eq.insert(-2, 'punching speed bag')
...
home_gym_eq.append('set of ding-dongs')  # Paul 10.05.2022 - for del purpose XD
...
home_gym_eq.append('rubber fitness balls')
# Paul 10.05.2022 - must .append() before .insert(len()),
# it seems that it override it at the end.
home_gym_eq.insert(len(home_gym_eq), '4 camera set with live tv '
                                     'screen - important!')
# Paul 10.05.2022 - inserting at the end with len()
home_gym_eq.insert(4, 'exercise mats')
home_gym_eq.insert(5, 'eliptical')
home_gym_eq.insert(2, 'magnetic treadmill')
home_gym_eq.insert(3, 'assault bike')
...
print('\nPrint popped "ab crunch machine" list:')
print(home_gym_eq)

# Paul 10.05.2022 - remove()
print('\nRemoved "rotary torso machine", "smith machine", '
      '"pull-ups machine" and del "set of ding-dongs":')
...
how_many_after = f'\nHow many equipments after adjustments are ' \
                 f'made: {len(home_gym_eq)}.'
print(how_many_after)

but_why = f"\nWhy? I tell you why - because " \
          f"{shitty0}, {shitty1}, {shitty2} and {shitty3} " \
          f"are useless shit. That's why."
print(but_why)

# Paul 10.05.2022 - now time to sort this list.

print(f'\nPRINT SORTED:\n {sorted(home_gym_eq)}')
print(f'\nPRINT SORTED IN REVERSE:\n{sorted(home_gym_eq, reverse=True)}')
# Paul 10.05.2022 - HA!
